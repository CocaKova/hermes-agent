        ...
        with self._init_lock:
            ...
            try:
                ...
                if not self._cfg["neo4j_password"]:
                    logger.warning("Graphiti: no neo4j password in env — not initializing")
                    return
                self._loop = _LoopThread()
                self._build()
                self._initialized = True
                logger.info("Graphiti memory initialized (group=%s, neo4j=%s)",
                            self._group_id, self._cfg["neo4j_uri"])
